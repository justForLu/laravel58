...
class FileController extends BaseController
{
    ...
    public function uploadPic(Request $request){
        ...
        $rootPath = Config::get('home.uploadImg','');
        $savePath = date('Y-m-d');
...
//        }
        ...
        if ($filename) {
            ...
            //新建文件夹（如果文件夹不存在）
            $folder = "upload/home/videos/" . date('Y-m-d',time());
            if (!file_exists($folder)){
                mkdir($folder,0777,true);
            }
            move_uploaded_file($_FILES["file"]["tmp_name"],
                "upload/home/videos/" . date('Y-m-d',time()) . "/" . $filename);

            $path = "/upload/home/videos/" . date('Y-m-d',time()) . "/"  . $filename;
            return response()->json($path);
        }
